...
-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:ns0="http://tei-c.org/TextComparator" exclude-result-prefixes="tei ns0">
	...
	<xsl:template match="tei:app">
		<xsl:choose>
			<xsl:when test="child::tei:rdg/tei:l">
				<div>
					<xsl:attribute name="class">
						<xsl:choose>
							...
							<xsl:when test="substring(@type, 1, 3) = 'typ'">
								<xsl:text>typographic </xsl:text>
							</xsl:when>
							<xsl:otherwise>
								<xsl:text>orthographic </xsl:text>
							</xsl:otherwise>
						</xsl:choose>
						<xsl:text>tei teiVariant variantScrollTarget tooltiptrigger ttVariant </xsl:text>
		...
		<xsl:choose>
			<xsl:when test="ancestor::tei:div[@type = 'collection']">
				<xsl:call-template name="variantMouseOver_collection"/>
			</xsl:when>
			<xsl:otherwise>
		...
		<span class="tei teiVariant anchor">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="substring(@id, string-length(@id) - string-length('a') + 1) = 'a'">
							<img src="assets/images/ms_arrow_right.svg" alt="pil höger" loading="lazy"/>
						</xsl:when>
						<xsl:otherwise>
							<img src="assets/images/ms_arrow_left.svg" alt="pil vänster" loading="lazy"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:otherwise>
			</xsl:choose>
	...
	<xsl:template match="tei:seg">
		<xsl:choose>
			<xsl:when test="ancestor::tei:rdg">
				<span class="tei teiVariant identical">
					<xsl:apply-templates/>
				</span>
			</xsl:when>
			<xsl:otherwise>
				<span>
					<xsl:attribute name="class">
						<xsl:text>tei teiVariant anchor</xsl:text>
					</xsl:attribute>
					<xsl:text>&#8968;</xsl:text>
				</span>
				<xsl:apply-templates/>
		...
		<xsl:choose>
			<xsl:when test="@type = 'empty'">
				<span class="tei teiVariant seg">
					<img src="assets/images/squared_times.svg" alt="normalisering" loading="lazy"/>
				</span>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates/>
...
<xsl:apply-templates/>
	...
	<!--
		The media_collection for these images has id 19 in the database.
		This maps to the /media/verk/ folder in this repository.
	-->
	<xsl:template match="tei:graphic">
		<img>
			<xsl:attribute name="src">
				<xsl:value-of select="$backendMediaBaseURL"/>
				<xsl:text>19/</xsl:text>
				<xsl:value-of select="@url"/>
			</xsl:attribute>
			<xsl:attribute name="class">
				<xsl:text>tei teiVariant tit_figure_graphic</xsl:text>
			</xsl:attribute>
			<xsl:attribute name="alt">
